...
link1_status=$(curl -Is https://tb.rg-adguard.net/dl.php?go=b9bfeeaf | grep HTTP | cut -f2 -d" " | head -1)
link2_status=$(curl -Is https://tb.rg-adguard.net/dl.php?go=b9bfeeaf | grep HTTP | cut -f2 -d" ")
#sudo wget -P /mediabots https://tb.rg-adguard.net/dl.php?go=b9bfeeaf # Windows Server 2012 R2 
if [ $link1_status = "200" ] ; then 
	sudo wget -O /mediabots/WS2012R2.ISO https://tb.rg-adguard.net/dl.php?go=b9bfeeaf 
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /mediabots https://tb.rg-adguard.net/dl.php?go=b9bfeeaf
else
