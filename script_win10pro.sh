...
link2_status=$(curl -Is http://51.38.54.218/win10pro.iso | grep HTTP | cut -f2 -d" ")
#sudo wget -P /mediabots http://51.38.54.218/win10pro.iso # Windows Server 2012 R2 
if [ $link1_status = "200" ] ; then 
	sudo wget -O /mediabots/WS2012R2.ISO http://51.38.54.218/win10pro.iso 
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /mediabots http://51.38.54.218/win10pro.iso
else
...
sudo wget -P /floppy http://51.38.54.218/PROWinx64.exe # Intel Network Adapter for Windows Server 2012 R2 
