...
link1_status=$(curl -Is https://releases.ubuntu.com/14.04/ubuntu-14.04.6-server-amd64.iso | grep HTTP | cut -f2 -d" " | head -1)
link2_status=$(curl -Is https://releases.ubuntu.com/14.04/ubuntu-14.04.6-server-amd64.iso | grep HTTP | cut -f2 -d" ")
#sudo wget -P /mediabots https://archive.org/download/WS2012R2/WS2012R2.ISO # Windows Server 2012 R2 
if [ $link1_status = "200" ] ; then 
	sudo wget -O /mediabots/WS2012R2.ISO https://releases.ubuntu.com/14.04/ubuntu-14.04.6-server-amd64.iso 
elif [ $link2_status = "200" -o $link2_status = "301" -o $link2_status = "302" ] ; then 
	sudo wget -P /mediabots https://releases.ubuntu.com/14.04/ubuntu-14.04.6-server-amd64.iso
else
...
sudo wget -P /floppy https://ftp.mozilla.org/pub/firefox/.exe
...
sudo wget -P /virtio https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/stable-virtio/vir
